...
import { filterUser, searchUser, updateUserStatus } from '../../services/admin/userListingService';
...
const DataTable: React.FC<DataTableProps> = ({ title, subtitle, data, type, fetchData, currentPage, totalPages, setCurrentPage, startIndex }) => {
  ...
  function filter(status: string) {
    if (type == 'user') {
      filterUser(status == 'active' ? 'unblock' : status == 'inactive' ? 'block' : 'all').then((response) => {
        setFilteredData(response.data.data)
      })
    } else {
      filterLawyer(status == 'active' ? 'unblock' : status == 'inactive' ? 'block' : 'all').then((response) => {
        setFilteredData(response.data.data)
      })
    }
  }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 p-6">
          <div className="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
            ...
            <div className="relative flex-1 max-w-md flex">
              ...
              <button onClick={searchData} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-r-lg text-sm font-medium transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 shadow-sm border border-blue-600">
                Search
              </button>
            </div>
            ...
            <div className="relative">
              <select onChange={(e) => filter(e.target.value)} className="appearance-none bg-white border border-gray-300 rounded-lg px-4 py-3 pr-10 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition duration-150 ease-in-out min-w-32">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredData.map((item, index) => (
                  <tr
                    key={item._id}
                    className={`hover:bg-gray-50 transition-colors duration-150 ease-in-out ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'
                      }`}
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${!item.status
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button
                        onClick={() => toggleStatus(item._id, item.status ? 'unblock' : 'block')}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 shadow-sm ${!item.status
