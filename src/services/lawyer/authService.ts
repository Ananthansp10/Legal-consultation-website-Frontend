...
export const register = async (data: FormData) => {
  try {
    const result = await axios.post(LAWYER_AUTH_API.SIGNUP, data, {
      headers: {
...
export const signin = async (data: { email: string; password: string }) => {
  try {
    const result = await axios.post(LAWYER_AUTH_API.SIGNIN, data);
    return result;
  } catch (error) {
    throw error;
  }
...
export const logout = async () => {
  try {
    const result = await axios.post(LAWYER_AUTH_API.LOGOUT);
    return result;
  } catch (error) {
    throw error;
  }
