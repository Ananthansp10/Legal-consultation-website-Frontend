...
export const LAWYER_API = {
  ...
  GET_APPOINTMENTS: (lawyerId: string, appointmentStatus: string, startIndex: number, limit: number) => `/lawyer/get-appointments/${lawyerId}/${appointmentStatus}/${startIndex}/${limit}`,
  ...
  ADD_PLAN: (lawyerId: string, planId: string, price: number) => `/lawyer/add-plan/${lawyerId}/${planId}/${price}`,
