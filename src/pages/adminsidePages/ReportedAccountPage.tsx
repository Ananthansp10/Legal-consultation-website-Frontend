...
const ReportedAccountsPage = () => {
  ...
  const [reasonFilter, setReasonFilter] = useState<
    "All" | "Harassment" | "Fake Profile" | "Scam" | "Other"
  >("All");
  const [selectedAccount, setSelectedAccount] =
    useState<ReportedAccount | null>(null);
  ...
  const tabs = [
    {
      key: "All" as const,
      label: "All",
      count: activeTab == "All" ? reports.length : 0,
    },
    {
      key: "users" as const,
      label: "Users",
      count:
        activeTab == "users"
          ? reports.filter((report) => report.userType == "user").length
          : 0,
    },
    {
      ...
      count:
        activeTab == "lawyers"
          ? reports.filter((report) => report.userType == "lawyer").length
          : 0,
    },
  ];
          ...
          <div className="space-y-4">
            {reports.map((account) => {
              ...
              return (
                <div
                  key={account._id}
                  className="bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 p-6 hover:shadow-xl hover:bg-white/70 transition-all duration-300"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      {/* Status Badge */}
                      <div
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${statusConfig.textColor} ${statusConfig.bgColor}`}
                      >
                        {statusConfig.icon}
                        {statusConfig.text}
                      </div>
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {selectedAccount.userType} • {selectedAccount.reportsCount}{" "}
                    total reports
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto max-h-96">
              <div className="space-y-4">
                {selectedAccount.reports.map((report, index) => (
                  <div
                    key={index}
                    className="bg-white/50 backdrop-blur-sm rounded-2xl p-5 border border-gray-200/50"
                  >
                    {/* Report Header */}
