...
    if (!formData.workLocation.trim()) newErrors.workLocation = "Work location required.";
    if (formData.availableDays.length === 0) newErrors.availableDays = "Select at least one day.";
    if (!formData.documents.barCouncilId) newErrors.barCouncilId = "Bar council ID is required.";
...
    if (!formData.documents.idProof) newErrors.idProof = "ID proof is required.";
    if (!formData.documents.experienceCertificate) newErrors.experienceCertificate = "Expereience certificate is required"
    if (!formData.gender.trim()) newErrors.gender = "Gender is required"
    if (!formData.dateOfBirth.trim()) newErrors.dob = "Date of birth is required"
    if (!formData.street.trim()) newErrors.street = "street is required"
    if (!formData.city.trim()) newErrors.city = "City is required"
    if (!formData.state.trim()) newErrors.state = "State is required"
    if (!formData.country.trim()) newErrors.country = "Country is required"
    if (!formData.lawFirm.trim()) newErrors.lawFirm = "Law firm is required"
    if (!formData.timeSlotStart.trim()) newErrors.timeSlotStart = "start time is required"
    if (!formData.timeSlotEnd.trim()) newErrors.timeSlotEnd = "End time is required"
    if (formData.languages.length==0) newErrors.languages = "Languages is required"
    if (!formData.profileImage) newErrors.profileImage = "profile Image is required"
    if (new Date().getFullYear()-new Date(formData.dateOfBirth).getFullYear() <18) newErrors.dob = "Date of birth should be greater than or equal to 18 years"
    // Example for education
    formData.education.forEach((edu, i) => {
      if (!edu.degree) newErrors[`education-degree-${edu.id}`] = "Degree required.";
      if (!edu.university) newErrors[`education-university-${edu.id}`] = "University required.";
      if (!edu.year || !/^\d{4}$/.test(edu.year)) newErrors[`education-year-${edu.id}`] = "Valid year required.";
    });
...
                <p className="text-xs text-red-500 mt-1">{errors.practiceAreas}</p>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
...
                  <p className="text-xs text-red-500 mt-1">{errors.barRegistrationNumber}</p>
                )}
              </div>
              <div>
...
                  <p className="text-xs text-red-500 mt-1">{errors.consultationFee}</p>
                )}
              </div>
              <div>
...
                />
                {errors.workLocation && (
                  <p className="text-xs text-red-500 mt-1">{errors.workLocation}</p>
                )}
              </div>
            </div>
            <div className="mt-6">
...
                <p className="text-xs text-red-500 mt-1">{errors.availableDays}</p>
              )}
            </div>
            <div className="mt-6">
...
                    <p className="text-xs text-red-500 mt-1">{errors.timeSlotStart}</p>
                  )}
                </div>
                <div>
...
                  </div>
                  {errors.timeSlotEnd && (
                    <p className="text-xs text-red-500 mt-1">{errors.timeSlotEnd}</p>
                  )}
                </div>
              </div>
            </div>
...
                      <p className="text-xs text-red-500 mt-1">{errors[`education-degree-${edu.id}`]}</p>
                    )}
                  </div>
                  <div>
                    <input
...
                      <p className="text-xs text-red-500 mt-1">{errors[`education-university-${edu.id}`]}</p>
                    )}
                  </div>
                  <div>
                    <input
...
                      <p className="text-xs text-red-500 mt-1">{errors[`education-year-${edu.id}`]}</p>
                    )}
                  </div>
