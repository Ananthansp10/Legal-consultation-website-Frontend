...
function SubscriptionPlanPage() {
  ...
  const getPlanIcon = (planName: string, planType: string, index: number) => {
    ...
    } else if (name.includes('enterprise') || name.includes('business') || type.includes('enterprise')) {
      return <Briefcase className="w-6 h-6" />;
    } else if (name.includes('standard') || type.includes('standard')) {
      return <Shield className="w-6 h-6" />;
    } else {
      const icons = [<Star className="w-6 h-6" />, <Zap className="w-6 h-6" />, <Award className="w-6 h-6" />];
  ...
  const formatPrice = (price: number, currency: string = '₹', duration: number) => {
              ...
              >
                ...
                {plan.duration > 200 && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
                    <span className={`bg-gradient-to-r ${colorScheme.gradient} text-white px-6 py-2 rounded-full text-sm font-bold shadow-xl animate-pulse`}>
                      ⭐ Most Popular
                    </span>
                  </div>
                )}
                ...
                <div className={`
                  relative h-full p-8 rounded-3xl backdrop-blur-sm border-2 ${colorScheme.border} 
                  bg-gradient-to-br ${colorScheme.bgGradient} shadow-xl ${colorScheme.shadow}
                ...
                `}>
                  ...
                  <div className="flex items-center mb-6 relative z-10">
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-semibold ${colorScheme.accent}`}>{plan.planType}</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-6 z-50">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl border-2 border-gray-200/50 shadow-2xl max-w-lg w-full p-8 transform transition-all duration-300 scale-100">
            ...
            <div className="flex items-center mb-6">
              ...
              <div>
                <h3 className="text-3xl font-bold text-gray-800">{selectedPlan.name}</h3>
                <p className="text-blue-600 font-semibold">{selectedPlan.planType}</p>
              </div>
            </div>
